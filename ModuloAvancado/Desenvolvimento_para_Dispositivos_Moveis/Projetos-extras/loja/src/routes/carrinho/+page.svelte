<script>
  ...
  async function finalizarCompra() {
    const productIDs = carrinho.items.map((product) => product.id);
...
{#if carrinho.quantidade > 0}
  <br />
  Itens: {carrinho.quantidade}<br />
  <button onclick={() => carrinho.limpar()}> Limpar Carrinho </button>
  <ul>
    {#each carrinho.items as product}
      <li>
        <img
          width="100px"
          alt="imagem do produto"
          src="http://localhost:3000/images/{product.photo}"
          align="center"
        />
        {product.name}
      </li>
